...
Usage: create-project [OPTIONS] <name>
	...
	-w, --wordpress   Create a new WordPress project
...
project-remove() {
	...
	echo "Project has been removed. The document root & database still exists."
	exit 0
}
...
url="$name.local"
docroot="/var/www/$url/htdocs"
logroot="/var/www/$url/log"
laravel=0
...
if [ ! -d "$docroot" ]; then
	...
	sudo chown www-data:www-data -R $logroot
	sudo chmod 777 -R $docroot
fi
...
if [ "$wordpress" = 1 ]; then
	...
	echo -e "Creating database for WordPress...\n"
	mysql -u root -p"123" -e "CREATE DATABASE $name CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci" >/dev/null
	echo -e "Installing latest version of WordPress...\n"
	wp core download --path=$docroot
	#sudo cp wp-config-sample.php wp-config.php
	sudo chown www-data:www-data -R $docroot
	sudo chown www-data:www-data -R $logroot
	sudo chmod 777 -R $content
fi
...
sudo sed -i 's#template.logroot#'$logroot'#g' /etc/apache2/sites-available/$name.conf

echo "Adding $url to the /etc/hosts file..."

sudo sed -i '1s/^/127.0.0.1 '$url'\n/' /etc/hosts
sudo a2ensite $name.conf >/dev/null
sudo service apache2 reload
